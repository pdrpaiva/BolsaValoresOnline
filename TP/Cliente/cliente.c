...
void ClientCommands(ClientState* stateCli, TCHAR* command) {
    ...
    if (_tcsncmp(command, TEXT("login"), 5) == 0) {
        ...
        if (_stscanf_s(command, TEXT("login %49s %49s"), username, (unsigned)_countof(username), password, (unsigned)_countof(password)) == 2) {
            ...
            WriteToPipe(stateCli, &MsgToSend);
        }
        else {
...
void InitializeClientState(ClientState* stateCli) {
    stateCli->hPipe = INVALID_HANDLE_VALUE;
...
int _tmain(int argc, LPTSTR argv[]) {
    ...
    InitializeClientState(&stateCli);
    Msg MsgToSend;

    if (!stateCli.readEvent || !stateCli.writeEvent || !stateCli.shutdownEvent) {
    ...
    if (!SetNamedPipeHandleState(stateCli.hPipe, &dwMode, NULL, NULL)) {
        _tprintf(TEXT("SetNamedPipeHandleState failed. GLE=%d\n"), GetLastError());
        return -1;
    }

    HANDLE hThread = CreateThread(NULL, 0, ThreadClientReader, &stateCli, 0, NULL);
    if (hThread == NULL) {
    ...
    while (1) {
        ...
        if (_tcscmp(MsgToSend.msg, TEXT("exit")) == 0) {
            ...
            WriteToPipe(&stateCli, &msgResponse);
